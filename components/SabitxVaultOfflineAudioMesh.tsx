...
export default function SabitxVaultOfflineAudioMesh() {
  ...
  async function startListening() {
    setError(null);
    if (!secure) {
      setError("Blocked: mic requires HTTPS or localhost. Deploy on Vercel (HTTPS) or test on https://localhost.");
      return;
    }
    await primeAudio();
    const ctx = audioCtxRef.current!;
    try {
      if (perm === "denied") {
        setError("Mic permission denied. Allow microphone for sabitx.com in site settings, then reload.");
        return;
      }
      if (!navigator.mediaDevices?.getUserMedia) {
        setError("Browser does not support getUserMedia.");
  ...
  return (
    <div className="bg-neutral-950 text-neutral-100 p-4 rounded-2xl">
      ...
      {!secure && (
        <div className="rounded-lg border border-red-500/30 bg-red-900/20 p-2 text-sm mb-2">
          Not a secure context. Use HTTPS/localhost; browsers block mic otherwise.
        </div>
      )}
      {error && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-900/20 p-2 text-sm mb-2">{error}</div>
      ...
      <div className="grid gap-3">
        <div className="bg-neutral-900 rounded-xl p-3">
          ...
          <p className="mt-2 text-xs text-neutral-400">
            Permission: {perm}. Secure: {String(secure)}. If denied, allow mic for sabitx.com in browser settings and reload.
          </p>
        </div>
      </div>
    </div>
